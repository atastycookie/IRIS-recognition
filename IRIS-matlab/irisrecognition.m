        ...
        if exist('img')
            ...
            img = rgb2gray(RGB);
             if (exist('iris_database.dat')==2)
                ...
                prompt={sprintf('%s',messaggio,'Номер пользователя должен быть положительнвм натуральным числом, не более ',num2str(max_class))};
                ...
                if size(zparameter,1)~=0
                    ...
                    if (class_number<=0)||(class_number>max_class)||(floor(class_number)~=class_number)||(~isa(class_number,'double'))||(any(any(imag(class_number))))
                        warndlg(sprintf('%s','Номер пользователя должен быть положительнвм натуральным числом, не более ',num2str(max_class)),' Warning ')
                    else
                        disp('Обработка...пожалуйста, подождите');
                        if class_number==max_class;
                            % для данного пользователя еще не было создано
                            % базы данных
                            max_class = class_number+1;
                            features  = findfeatures(img,scales);
                        else
                            % для данного пользователя уже существует база
                            % данных
                            features  = findfeatures(img,scales);
                        end
                        ...
                        msgbox(sprintf('%s','Изображение было успешно добавлено в базу данных пользователя ',num2str(class_number)),'Database result','help');
                        close all;
                        clear('img');
                        clc;
                        disp('Изображение было успешно добавлено в базу данных');
                ...
                if size(zparameter,1)~=0
                    ...
                    if (class_number<=0)||(class_number>max_class)||(floor(class_number)~=class_number)||(~isa(class_number,'double'))||(any(any(imag(class_number))))
                        ...
                        msgbox(sprintf('%s','База данных была пуста. Изображение было успешно добавлено к базе двнных пользователя ',num2str(class_number)),'Database result','help');
                        clc;
                        close all;
                        clear('img');
